...
from board.chat_env import ChatEnv, ChatEnvConfig
from board.statistics import get_info
from board.utils import log_and_print_online, now
...
class ChatChain:
    ...
    def __init__(self,
        ...
        Args:
            ...
            task_prompt: the user input prompt for the task
            project_name: the user input name for the task
        ...
        self.chat_env_config = ChatEnvConfig(clear_structure=check_bool(self.config["clear_structure"]),
                                             brainstorming=check_bool(
                                                 self.config["brainstorming"]))
        ...
        # import all used phases in PhaseConfig.json from board.phase
        ...
        self.compose_phase_module = importlib.import_module(
            "board.composed_phase")
        self.phase_module = importlib.import_module("board.phase")
        self.phases = dict()
        for phase in self.config_phase:
            assistant_role_name = self.config_phase[phase]['assistant_role_name']
            user_role_name = self.config_phase[phase]['user_role_name']
            phase_prompt = "\n\n".join(
                self.config_phase[phase]['phase_prompt'])
    ...
    def execute_step(self, phase_item: dict):
        ...
        if phase_type == "SimplePhase":
            ...
            if phase in self.phases:
                ...
                raise RuntimeError(
                    f"Phase '{phase}' is not yet implemented in board.phase")
        # For ComposedPhase, we create instance here then conduct the "ComposedPhase.execute" method
            ...
            if not compose_phase_class:
                raise RuntimeError(
                    f"Phase '{phase}' is not yet implemented in board.compose_phase")
    ...
    def get_logfilepath(self):
        ...
        log_filepath = os.path.join(directory, "{}.log".format(
    ...
    def pre_processing(self):
        ...
        software_path = os.path.join(directory, "_".join(
            [self.project_name, self.org_name, self.start_time]))
    ...
    def post_processing(self):
        ...
        post_info += "Board Starts ({})".format(self.start_time) + "\n\n"
        post_info += "Board Ends ({})".format(now_time) + "\n\n"
...
then you should return a message in a format like \"<INFO> revised_version_of_the_description\", do not return messages in other formats.""".format(
        ...
        assistant_response, user_response = role_play_session.step(
            input_user_msg, True)
        revised_task_prompt = assistant_response.msg.content.split(
            "<INFO>")[-1].lower().strip()
