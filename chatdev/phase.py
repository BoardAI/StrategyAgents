...
class GoalSetting(Phase):
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0:
            chat_env.env_dict['vision'] = self.seminar_conclusion.split(
                "< >")[-1].lower().replace(".", "").strip()
        return chat_env


class ProductPlanning(Phase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

    def update_phase_env(self, chat_env):
        self.phase_env.update({"task": chat_env.env_dict['task_prompt'],
                               "vision": chat_env.env_dict['vision']})

    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0 and "<INFO>" in self.seminar_conclusion:
            chat_env.env_dict['product_plan'] = self.seminar_conclusion.split(
                "<INFO>")[-1].lower().replace(".", "").strip()
        elif len(self.seminar_conclusion) > 0:
            chat_env.env_dict['product_plan'] = self.seminar_conclusion
        else:
            chat_env.env_dict['product_plan'] = "NO MARKET PLAN"
        return chat_env


class SupplierPlanning(Phase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0 and "<INFO>" in self.seminar_conclusion:
            chat_env.env_dict['supplier_plan'] = self.seminar_conclusion.split(
                "<INFO>")[-1].lower().replace(".", "").strip()
        elif len(self.seminar_conclusion) > 0:
            chat_env.env_dict['supplier_plan'] = self.seminar_conclusion
        else:
            chat_env.env_dict['supplier_plan'] = "NO MARKET PLAN"
        return chat_env


class FinancePlanning(Phase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0 and "<INFO>" in self.seminar_conclusion:
            chat_env.env_dict['financial_plan'] = self.seminar_conclusion.split(
                "<INFO>")[-1].lower().replace(".", "").strip()
        elif len(self.seminar_conclusion) > 0:
            chat_env.env_dict['financial_plan'] = self.seminar_conclusion
        else:
            chat_env.env_dict['financial_plan'] = "NO MARKET PLAN"
        return chat_env


class MarketPlanning(Phase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0 and "<INFO>" in self.seminar_conclusion:
            chat_env.env_dict['market_plan'] = self.seminar_conclusion.split(
                "<INFO>")[-1].lower().replace(".", "").strip()
        elif len(self.seminar_conclusion) > 0:
            chat_env.env_dict['market_plan'] = self.seminar_conclusion
        else:
            chat_env.env_dict['market_plan'] = "NO MARKET PLAN"
        return chat_env


class ProductResearch(Phase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

    def update_phase_env(self, chat_env):
        self.phase_env.update({"task": chat_env.env_dict['task_prompt'],
                               "product_plan": chat_env.env_dict['product_plan'],
                               "vision": chat_env.env_dict['vision']})

    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0 and "<INFO>" in self.seminar_conclusion:
            chat_env.env_dict['product_research_summary'] = self.seminar_conclusion.split(
                "<INFO>")[-1].lower().replace(".", "").strip()
        elif len(self.seminar_conclusion) > 0:
            chat_env.env_dict['product_research_summary'] = self.seminar_conclusion
        else:
            chat_env.env_dict['product_research_summary'] = "NO MARKET PLAN"
        return chat_env


class SupplierResearch(Phase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

    def update_phase_env(self, chat_env):
        self.phase_env.update({"task": chat_env.env_dict['task_prompt'],
                               "supplier_plan": chat_env.env_dict['supplier_plan'],
                               "vision": chat_env.env_dict['vision']})

    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0 and "<INFO>" in self.seminar_conclusion:
            chat_env.env_dict['supplier_research_summary'] = self.seminar_conclusion.split(
                "<INFO>")[-1].lower().replace(".", "").strip()
        elif len(self.seminar_conclusion) > 0:
            chat_env.env_dict['supplier_research_summary'] = self.seminar_conclusion
        else:
            chat_env.env_dict['supplier_research_summary'] = "NO MARKET PLAN"
        return chat_env


class FinanceResearch(Phase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

    def update_phase_env(self, chat_env):
        self.phase_env.update({"task": chat_env.env_dict['task_prompt'],
                               "financial_plan": chat_env.env_dict['financial_plan'],
                               "vision": chat_env.env_dict['vision']})

    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0 and "<INFO>" in self.seminar_conclusion:
            chat_env.env_dict['financial_research_summary'] = self.seminar_conclusion.split(
                "<INFO>")[-1].lower().replace(".", "").strip()
        elif len(self.seminar_conclusion) > 0:
            chat_env.env_dict['financial_research_summary'] = self.seminar_conclusion
        else:
            chat_env.env_dict['financial_research_summary'] = "NO MARKET PLAN"
        return chat_env


class MarketResearch(Phase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

    def update_phase_env(self, chat_env):
        self.phase_env.update({"task": chat_env.env_dict['task_prompt'],
                               "market_plan": chat_env.env_dict['market_plan'],
                               "vision": chat_env.env_dict['vision']})

    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0 and "<INFO>" in self.seminar_conclusion:
            chat_env.env_dict['market_research_summary'] = self.seminar_conclusion.split(
                "<INFO>")[-1].lower().replace(".", "").strip()
        elif len(self.seminar_conclusion) > 0:
            chat_env.env_dict['market_research_summary'] = self.seminar_conclusion
        else:
            chat_env.env_dict['market_research_summary'] = "NO MARKET PLAN"
        return chat_env
...
class ContractAnalysis(Phase):
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0 and "<INFO>" in self.seminar_conclusion:
            chat_env.env_dict['legal_concerns'] = self.seminar_conclusion.split(
                "<INFO>")[-1].lower().replace(".", "").strip()
        elif len(self.seminar_conclusion) > 0:
            chat_env.env_dict['legal_concerns'] = self.seminar_conclusion
        else:
            chat_env.env_dict['legal_concerns'] = "NONE"
        return chat_env


class StrategyPlanning(Phase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def update_phase_env(self, chat_env):
        self.phase_env.update({"task": chat_env.env_dict['task_prompt'],
                               "vision": chat_env.env_dict['vision'],
                               "product_research_summary": chat_env.env_dict['product_research_summary'],
                               "supplier_research_summary": chat_env.env_dict['supplier_research_summary'],
                               "financial_research_summary": chat_env.env_dict['financial_research_summary'],
                               "market_research_summary": chat_env.env_dict['market_research_summary'],
                               "legal_concerns": chat_env.env_dict['legal_concerns']})

    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0 and "<INFO>" in self.seminar_conclusion:
            chat_env.env_dict['strategy_considerations'] = self.seminar_conclusion.split(
                "<INFO>")[-1].lower().replace(".", "").strip()
        elif len(self.seminar_conclusion) > 0:
            chat_env.env_dict['strategy_considerations'] = self.seminar_conclusion
        else:
            chat_env.env_dict['strategy_considerations'] = "NONE"
        return chat_env
